...
| type | data |
|---|---|
| `run.started` | `task{name,domain,n_cases,goal}`, `config{models,budget_usd,max_generations}` |
...
| `metrics.snapshot` | `generation`, `pass_rate`, `cost_usd`, `latency_s_mean`, `structural_fidelity`, `vs_baseline{pass_delta,cost_ratio}` |
...
### 4.1 `architect.py`
Input: task pack manifest (goal, tool manifest, answer format, 3 example cases). Output: `Architecture` for g000.
...
### 4.4 `diagnose.py`
Input: failed cases (inputs, answers, expected, per-role outputs), ablation table, generation history. Output: `diagnosis.emitted`.
...
### 4.7 `loop.py`
```
run(task_pack, config):
  emit run.started
  A = architect(task)            ; emit architecture.proposed g000
  for g in 0..max_generations:
    ...
    D = diagnose(R, T, history)  ; emit diagnosis.emitted
...
- `python_exec(code) -> stdout` — subprocess with timeout, no network, restricted imports. Required for SMFR arithmetic.
- `lookup_price(company, date, price_type) -> float` and `list_transactions(investor) -> [...]` — SMFR accessors over the case's haystack (see `02`).
- `calculator(expr) -> number` — MGSM.
- BFCL functions are *virtual*: the model emits a call; we don't execute, we AST-compare (see `02`).
