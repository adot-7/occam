...
- **`wasted_calls` is named in `02 §2` but never defined.** WP-04 records the raw fields it would be computed from — per call: `name`, `arguments`, `status`, `latency_s`, `bytes`, `cached`, `http_status`, `error` — and deliberately does not guess the aggregation. Whoever owns WP-08 should pin the definition (most likely failed calls plus redundant repeats of an identical request) in `03`.
